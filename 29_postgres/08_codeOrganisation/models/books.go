...
func UpdateBook(r *http.Request) (Book, error) {
	...
	_, err = config.DB.Exec(`UPDATE books SET title = $2, author = $3, price = $4 WHERE isbn = $1;`, bk.Isbn, bk.Title, bk.Author, bk.Price)
	if err != nil {
		return bk, err
	}
	return bk, nil
...
func DeleteBook(r *http.Request) error {
	...
	// delete book
	_, err := config.DB.Exec(`DELETE FROM book WHERE isbn = $1;`, isbn)
	if err != nil {
